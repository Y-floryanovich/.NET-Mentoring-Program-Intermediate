...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            var m1 = new Matrix(35, 35, true);
            var m2 = new Matrix(35, 35, true);
            var multiplierSync = new MatricesMultiplier();
            var multiplierParallel = new MatricesMultiplierParallel();
            Stopwatch stopWatch = new Stopwatch();
            stopWatch.Start();
            multiplierSync.Multiply(m1, m2);
            stopWatch.Stop();
            var resSync = stopWatch.ElapsedMilliseconds;

            Stopwatch stopWatchParallel = new Stopwatch();
            stopWatchParallel.Start();
            multiplierParallel.Multiply(m1, m2);
            stopWatchParallel.Stop();
            var resParallel = stopWatchParallel.ElapsedMilliseconds;


            Assert.IsTrue(resParallel > resSync);
        }
